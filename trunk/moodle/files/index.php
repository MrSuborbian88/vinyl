...
//  UPDATE - Daniel Ploch, 04/17/2009
	...
	switch ($action) {
		...
		case "upload":
			...
			if ($save and confirm_sesskey()) {
				...
				$um = new upload_manager($uploadarr,false,false,$course,false,0,false,false,$numfiles-1);
				$dir = "$basedir$wdir";
				...
				// VINL Code - Daniel Ploch, 04/17/2009
				//   This if statement, and multiple lines in the "echo" sequence below
				//   In particular, the addition of the "Upload More Files" button

				if ($addfiles)
				{
					if ($numfiles == 1) $numfiles = 10;
					else $numfiles = $numfiles + 10;
				}

				$struploadafile = get_string("uploadafile");
				$struploadthisfile = (($numfiles == 1) ? get_string("uploadthisfile") : get_string("uploadthesefiles"));
				$strmorefileslots = get_string("morefileslots");
				...
				upload_print_form_fragment($numfiles,array('userfile'),null,false,null,$upload_max_filesize,0,false);
				echo " </td></tr></table>";
				echo " <input type=\"submit\" name=\"save\" value=\"$struploadthisfile\" /><br />";
				echo "</div>";
				echo "</form>";
				echo "<form enctype=\"multipart/form-data\" method=\"get\" action=\"index.php\">";
				echo "<div>";
				echo ' <input type="hidden" name="choose" value="'.$choose.'" />';
				echo " <input type=\"hidden\" name=\"id\" value=\"$id\" />";
				echo " <input type=\"hidden\" name=\"wdir\" value=\"$wdir\" />";
				echo " <input type=\"hidden\" name=\"action\" value=\"upload\" />";
				echo " <input type=\"hidden\" name=\"numfiles\" value=\"$numfiles\" />";
				echo " <input type=\"submit\" name=\"addfiles\" value=\"$strmorefileslots\" \>";
				echo "</div>";
				echo "</form>";
