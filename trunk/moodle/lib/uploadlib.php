...
class upload_manager {
	...
	*/
	...
	function acceptExtra($string)
	{
		for ($i = 1; $i <= $extrainput; $i++)
			if ("FILE_'.$i.'" == $string) return true;
		return false;
	}
	 ...
	 */
	function upload_manager($inputname='', $deleteothers=false, $handlecollisions=false, $course=null, $recoverifmultiple=false, $modbytes=0, $silent=false, $allownull=false, $allownullmultiple=true, $extrainputs=0) {
	...
	// VINL Code
	//   Daniel Ploch 04/17/2009
	// - Added parameter to "preprocess_files" to accept additional inputs
	...
	function preprocess_files() {
		...
		foreach ($_FILES as $name => $file) {
			...
			if (empty($this->inputname) || $name == $this->inputname || ($extrainput != 0 && acceptExtra($name))) { // if we have input name, only process if it matches.
